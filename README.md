...
conky.config = {
    -- Позиция: верхний левый угол, без отступов
    alignment = 'top_left',
    gap_x = 0,
    gap_y = 0,

    -- Размер полосы: вся ширина экрана (1920), высота 20px
    minimum_width = 1920,
    maximum_width = 1920,
    minimum_height = 20,
    ...
    -- Собственное окно
    own_window = true,
    own_window_type = 'dock',          -- панель сверху, резервирует место (правило KWin держит выше панели)
    own_window_hints = 'above,sticky',
    own_window_transparent = true,
    own_window_argb_visual = true,
    own_window_argb_value = 0,         -- 0 = полностью прозрачный фон (при transparent=true не используется)
    own_window_colour = '000000',      -- чёрный фон полосы
    ...
    -- Lua-скрипт: трансформ вывода ядра (cachyos -> arch1-1)
    lua_load = '/home/topsh/.config/conky/kernel.lua',
}

-- Каждый модуль начинается с абсолютного ${goto N} => не зависит от ширины соседей
conky.text = [[
